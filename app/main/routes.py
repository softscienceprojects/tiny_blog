...
def index():
    ...
    posts = current_user.followed_posts().paginate(
        page, current_app.config['POSTS_PER_PAGE'], False) #same level as 'return' so we can reference it in index
...
def explore():
    ...
    return render_template('index.html', title='Explore', posts=posts.items, next_url=next_url, prev_url=prev_url) #just reuse the index template


@bp.route('/user/<username>') #dynamic content in <>
@login_required
def user(username):
